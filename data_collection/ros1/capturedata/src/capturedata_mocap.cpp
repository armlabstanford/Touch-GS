...
//   message_filters::Subscriber<sensor_msgs::TimeReference> cmd_sub(n, "/capturedata/lib_cmd", 1);
  message_filters::Subscriber<geometry_msgs::PoseStamped> pose_sub(n, "/box/mavros/vision_pose/pose", 1);
  message_filters::Subscriber<geometry_msgs::PoseStamped> pose_sub_cam(n, "/camera_frame/mavros/vision_pose/pose", 1);
    ...
    sub_pose_obj_.subscribe(n, "/box/mavros/vision_pose/pose", 1);
    sub_pose_cam_.subscribe(n, "/camera_frame/mavros/vision_pose/pose", 1);


    sync_.reset(new Sync(cap_syncPolicy(100), sub_tact_, sub_webcam_, sub_depth_, sub_tact_depth_, sub_pose_obj_, sub_pose_cam_));
    sync_->registerCallback(boost::bind(&Save::Callback_assorted, this, _1, _2, _3, _4, _5, _6));
...
void Save::Callback_assorted(const sensor_msgs::ImageConstPtr &img_tact, 
                        ...
                        const geometry_msgs::PoseStamped::ConstPtr& msg_pose,
                        const geometry_msgs::PoseStamped::ConstPtr& msg_pose_cam){
//// 
...
////
	...
	if (rwFlag == 1)
	{
        ...
        write_pose(msg_pose_cam, filename, timestr);

        // save pose of object 
        filename = ros::package::getPath("capturedata")+"/data/object_pose.csv";
        write_pose(msg_pose, filename, timestr);
        // gettfframe_object(filename);
