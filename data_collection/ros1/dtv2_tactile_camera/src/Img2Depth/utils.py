...
from networks.DenseNet_loss import ssim as ssim_criterion
from networks.DenseNet_loss import depth_loss as gradient_criterion
...
def compute_errors_incl1(gt, pred):
    thresh = np.maximum((gt / pred), (pred / gt))
    d1 = (thresh < 1.25).mean()
    d2 = (thresh < 1.25 ** 2).mean()
    ...
    return [silog, abs_rel, log10, rms, sq_rel, log_rms, d1, d2, l1]

def compute_errors_force(gt, pred):
    thresh = np.maximum((gt / pred), (pred / gt))
    d1 = (thresh < 1.25).mean()
    d2 = (thresh < 1.25 ** 2).mean()
    d3 = (thresh < 1.25 ** 3).mean()
...
class silog_loss_nomask(nn.Module):
    def __init__(self, variance_focus):
        super(silog_loss_nomask, self).__init__()
        self.variance_focus = variance_focus
...
class prev_net_loss(nn.Module):
    def __init__(self, variance_focus, theta):
        super(prev_net_loss, self).__init__()
        self.variance_focus = variance_focus
    ...
    def forward(self, depth_est, depth_gt):
        d_e = depth_est
        d_gt = depth_gt
        l1_loss =self.l1_crit(d_e, d_gt)
        l2_loss =self.l2_crit(d_e, d_gt)
        ssim_loss = torch.clamp((1-ssim_criterion(d_e, d_gt, 256.0/1.0))*0.5, min=0, max=1)
        gradient_loss = gradient_criterion(d_gt, d_e, device=depth_est.device)

        # net_loss = (1.0 * ssim_loss) + (1.0 * torch.mean(gradient_loss)) + (self.theta * torch.mean(l1_loss))
        net_loss = (1.0 * ssim_loss) +  (self.theta * torch.mean(l1_loss)) + (1.0 * torch.mean(gradient_loss)) 

        return net_loss

class prev_net_loss_ch(nn.Module):
    def __init__(self, variance_focus, theta):
        super(prev_net_loss_ch, self).__init__()
        self.variance_focus = variance_focus
        self.l1_crit = nn.L1Loss()
        self.l2_crit = nn.MSELoss()
        self.theta = theta
    ...
    def forward(self, depth_est, depth_gt):
        ...
        # net_loss = (1.0 * ssim_loss) + (1.0 * torch.mean(gradient_loss)) + (self.theta * torch.mean(l1_loss))
        net_loss = (1.0 * ssim_loss) +  (self.theta * torch.mean(l1_loss)) + (self.theta * torch.mean(l2_loss)) 

        return net_loss

class rmslog_loss(nn.Module):
    def __init__(self, variance_focus):
        super(rmslog_loss, self).__init__()
        self.variance_focus = variance_focus

    def forward(self, depth_est, depth_gt, mask):
        d = torch.log(depth_est[mask]) - torch.log(depth_gt[mask])
        return torch.sqrt((d ** 2).mean()) 
...
class rmseloss(nn.Module):
    ...
    def forward(self, force_est, force_gt):
        rms = (force_gt - force_est) ** 2
